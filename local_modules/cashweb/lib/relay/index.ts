...
export class ReadOnlyRelayClient {
  ...
  async getRelayData(address: string | Address) {
    ...
    assert(
      validateBinary(response.data),
      "invalid type for legacy address payload"
    );
    ...
    try {
      ...
      Promise.all(
        ...
        .then(async () => {
          ...
          await Promise.all(
            stagedUtxos.map((utxo) =>
              wallet.storage.deleteOutpoint(calcId(utxo))
            )
          );
        })
        .catch(async (err) => {
      ...
      const payloadDigestHex =
        validateError(err) && err.payloadDigest?.toString("hex");
      this.events.emit("messageSendError", {
        address,
        senderAddress,
        index: payloadDigestHex,
