...
export class RelayClient {
  ...
  sendMessageImpl ({ addr, items, stampAmount }) {
    ...
    // We don't need to await this, just use normal promise resolution here for reporting errors back.
    this.pushMessages(destAddr, messageSet).then(() => {
      this.events.emit('messageSent', { addr, index: payloadDigestHex, items, outpoints })
    }).catch((err) => {
      console.error(err)
      if (err.response) {
      ...
      this.events.emit('messageSendError', { addr, index: payloadDigestHex, items })
    })
  }
