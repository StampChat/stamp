...
export class RelayClient {
  ...
  setUpWebsocket (addr) {
    ...
    socket.onmessage = event => {
      ...
      for (let index in messageList) {
        ...
        this.receiveMessage({ serverTime, receivedTime, message }).then(() => console.log('message received')).catch(err => console.error(err))
      }
    }
  ...
  async getRawPayload (addr, digest) {
    ...
    console.log('getrawpayload', digest)
  ...
  async putProfile (addr, metadata) {
    console.log(addr, metadata, this.token)
  ...
  async sendMessage ({ addr, text, replyDigest, stampAmount }) {
    ...
    console.log(addr, text, replyDigest, stampAmount, destPubKey)
  ...
  async receiveMessage ({ serverTime, receivedTime, message }) {
    ...
    if (payloadDigestFromServer.length !== 0) {
      ...
      console.log('dont have', payloadDigestHex)
    }
  ...
  async refresh ({ lastReceived } = {}) {
    ...
    console.log('refreshing', lastReceived, wallet, myAddressStr)
