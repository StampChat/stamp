...
import { SoapboxMessage, SoapboxMessageEntry } from 'src/cashweb/types/soapbox'

type MessageWithReplies = SoapboxMessage & { replies: MessageWithReplies[] }
...
const module: Module<State, unknown> = {
  ...
  mutations: {
    setEntries (state, messages: SoapboxMessage[]) {
    ...
    setMessage (state, message: SoapboxMessage) {
  ...
  actions: {
    ...
    async putMessage ({ dispatch }, { wallet, entry, satoshis, topic, parentDigest }: { wallet: Wallet, entry: SoapboxMessageEntry, satoshis: number, topic: string, parentDigest?: string }) {
      const keyserver = new KeyserverHandler({ wallet, networkName: displayNetwork, keyservers })
      console.log('fetching messages')
