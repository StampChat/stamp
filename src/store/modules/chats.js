export default {
    ...
    receiveMessage (state, { addr, text, timestamp }) {
        ...
        // Add new message
        let newMsg = { outbound: false, sent: true, body: text, timestamp } state
                       .data[addr]
        .messages.push(newMsg)
},
...
actions: {
    ...
    async refresh({ commit, rootGetters, getters, dispatch }) {
        ...
        let myAddressStr = rootGetters['wallet/getMyAddressStr'] let client =
            rootGetters['relayClient/getClient'] let lastReceived =
                getters['getLastReceived'] || 0
