export default {
  ...
  mutations: {
    ...
    reset(state) {
      ...
      state.addresses = {} state.totalBalance = 0
    },
    ...
    reinitialize(state) {
      if (state.xPrivKey != null) {
        ...
        state.identityPrivKey =
            cashlib.PrivateKey.fromObject(state.identityPrivKey)
      }
    }
  },
  actions: {
    ...
    async startListeners({dispatch}) {
      let client = this.state.electrumHandler.client await client.subscribe.on(
          'blockchain.address.subscribe',
          async (result) => {
              let address = result[0] await dispatch('updateBalance', address)})
