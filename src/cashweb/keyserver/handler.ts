...
import { BroadcastEntry, BroadcastMessage, SoapboxPost } from './broadcast_pb'
import { SoapboxMessage, SoapboxMessageEntry } from '../types/soapbox'
...
export class KeyserverHandler {
  ...
  async createBroadcast (topic: string, entries: SoapboxMessageEntry[], vote: number, parentDigest?: string) {
    assert(this.wallet, 'Missing wallet while running updateKeyMetadata')
  ...
  parseWrapper (wrapper: AuthWrapper) {
    ...
    const parsedEntries: SoapboxMessageEntry[] = []
    ...
    const parsedMessage: SoapboxMessage = {
      poster: address,
    ...
    for (const entry of entries) {
      ...
      if (kind === 'post') {
        const post = SoapboxPost.deserializeBinary(payload)
        parsedEntries.push({
          kind: 'post',
          title: post.getTitle(),
          url: post.getUrl(),
